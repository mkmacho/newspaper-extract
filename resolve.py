...
def geoapify_request(query, biggest_nearby_cities, timeout=10):
    ...
    ## Pro API:
    url = "https://bk01.geoapify.net/v1/geocode/search?text={}&apiKey={}".format(query, API_KEY)
    ## Demo API:
    ## url = "https://api.geoapify.com/v1/geocode/search?text={}&apiKey={}".format(query, API_KEY)
    response = get_wrapper(url, timeout=timeout)
    assert isinstance(response, dict)
...
def resolve(address_dicts_list:list, US_DATA:object, nominatum=False, geoapify=True, verbose=False):
    ...
    if verbose and nominatum and geoapify:
...
if __name__ == "__main__":
    ...
    os.environ['GEOAPIFY_API_KEY'] = '7246d172308f4b4c80d54cccf2286ed7'
