...
if __name__ == "__main__":
    ...
    parser.add_argument('--extract_address', type=int, default=0)
    ...
    if args.multiprocessing:
        ...
        if args.extract_wage:
            ...
            for batch_idx in range(ceil(len(sample) / args.batch_size)):
                ...
                # wages_batch.to_csv(add_filepath_suffix(args.output_dir, paper, 
                #     n=(batch_idx+1)*args.batch_size, suffix='extract-batch', ext='csv'))
        ...
        if args.extract_wage:
            ...
            for batch_idx in range(ceil(len(sample) / args.batch_size)):
                ...
                #     n=(batch_idx+1)*args.batch_size, suffix='extract-batch', ext='csv'))
            sample = sample.join(wages)
            # wages = sample.raw_content.apply(NEWSPAPER.employer_info).to_list()
            # sample = sample.join(pd.DataFrame(wages, index=sample.index))
    
    # preds = sample.apply(lambda row: NEWSPAPER.populate_ad(row), axis=1, result_type='expand')
    # preds = pd.concat([sample, preds], axis='columns')
